  ...
  return (
    <div className="bg-[#0D0F0E] px-3 sm:px-6 py-3">
      <div className="max-w-4xl mx-auto">
        ...
        {/* ── Top row: Prompt tabs (outside/above box) + Model dropdown ── */}
        <div className="flex items-center justify-between mb-1.5 px-1">
          {/* Left: Positive / Negative tabs */}
          <div className="flex items-center gap-0.5 bg-[#1a1d1b] rounded-lg p-0.5">
            <button
              onClick={() => setActiveTab('positive')}
              className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md transition-all ${
                activeTab === 'positive'
                  ? 'bg-[#2a2e2c] text-white shadow-sm'
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md transition-all relative ${
                activeTab === 'negative'
                  ? 'bg-[#2a2e2c] text-white shadow-sm'
                  : 'text-gray-500 hover:text-gray-300'
              }`}
            >
              − Negative
              {hasNegativePrompt && (
                <span className="w-1.5 h-1.5 rounded-full bg-purple-400 inline-block ml-1" />
              )}
            </button>
          </div>
          ...
          <div className="relative" ref={modelDropdownRef}>
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium text-gray-300 hover:text-white bg-[#1a1d1b] hover:bg-white/10 border border-white/10 px-3 py-1.5 rounded-lg transition-all"
            >
        ...
        <div className="relative bg-[#141816] border border-white/10 rounded-2xl overflow-visible focus-within:border-purple-500/40 transition-colors">
          ...
          <div className="flex items-center justify-between gap-2 px-3 pb-3 flex-wrap sm:flex-nowrap">
            ...
            {/* Left: Settings popup button + Free gen toggle */}
            <div className="flex items-center gap-2">

              {/* Generation Settings button — opens aspect ratio popup */}
              <div className="relative" ref={settingsRef}>
                ...
                {/* Settings popup — aspect ratio */}
                {settingsOpen && (
                  <div className="absolute bottom-full left-0 mb-2 bg-[#1a1d1b] border border-white/10 rounded-xl shadow-2xl p-4 z-50 min-w-[220px] animate-in fade-in slide-in-from-bottom-2 duration-150">
                    <p className="text-xs text-gray-400 font-medium mb-3">Aspect Ratio</p>
                    <div className="flex flex-wrap gap-1.5">
                      {ASPECT_RATIOS.map(r => (
                        <button
                          key={r.value}
                          onClick={() => { onRatioChange(r.value); setSettingsOpen(false); }}
                    ...
                    <p className="text-[10px] text-gray-600 mt-3">Current: <span className="text-gray-400">{selectedRatio}</span></p>
                  </div>
                )}
              </div>

              {/* Free Generation Toggle — visible to all, only Standard can enable */}
              <button
